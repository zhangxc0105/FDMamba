...
class WarmupHalfCycleCosine(_LRScheduler):
    """ Warmup with half-cycle cosine learning rate scheme.
    ...
    def __init__(self,
        ...
        self.max_lr = max_lr
        super(WarmupHalfCycleCosine, self).__init__(optimizer, last_epoch)

    def get_lr(self):
        if self.last_epoch < self.warmup_epochs:
            # Linear warmup
            lr = self.max_lr * (self.last_epoch + 1) / self.warmup_epochs
        else:
            # Half-cycle cosine decay
            lr = self.max_lr * 0.5 * (1. + math.cos(math.pi * (self.last_epoch - self.warmup_epochs) / (self.total_epochs - self.warmup_epochs)))
