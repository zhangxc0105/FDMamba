...
class newLoss(nn.Module):
    ...
    def forward(self, input, target):
        ...
        y = ((del_x * 255) ** self.r1 / 255) * del_x * 1
        return torch.sum(y)
...
def save_config(time, log, log_dir='log/'):
    open_type = 'a' if os.path.exists(get_path('./' + log_dir + str(time) + '/records.txt')) else 'w'
    log_file = open(get_path('./' + log_dir + str(time) + '/records.txt'), open_type)
    log_file.write(str(log) + '\n')


def save_net_config(time, log, log_dir='log/'):
