...
from hqnr.HQNR import HQNR
from hqnr.my_D_s import D_s as my_Ds
from hqnr.imresize import imresize
...
def mtf_resize(img, satellite='QB', scale=4):
    ...
    if satellite == 'QB':
        ...
        GNyq = [0.3, 0.3, 0.3, 0.3]  # Band Order: B,G,R,NIR
        GNyqPan = 0.15
    else:
...
if __name__ == '__main__':
    ...
    base_dir = r'D:\MetricCode\2_DL_Result\PanCollection\WV3_Full'
    output_dir = 'HQNR_HeatmapsWV3'
    os.makedirs(output_dir, exist_ok=True)

    model_dirs = [
        # 'BDSD_PC', 'cannet',  'FusionNet',
        # 'MTF_GLP_FS', 'PanMamba', 'PanNet', 'PNN', 'TV',#,
        # 'CCFNet',
        # 'U2NET', 'Premix'#, 'Ramsf', 'ADWM'
        'fdmamba'
    ]

    input_base = r'D:\pansharpening\DLPan-Toolbox-main\02-Test-toolbox-for-traditional-and-DL(Matlab)\1_TestData\Datasets Testing\wv3fr'
    ...
    for index in range(0, 20):
        ...
        # 加载 I_MS 和 I_PAN
        input_mat_path = os.path.join(input_base, f'Test(HxWxC)_wv3_data_fr{index + 1}.mat')
        if not os.path.exists(input_mat_path):
            print(f"Input MAT not found: {input_mat_path}")
            continue
        ...
        for model in model_dirs:
            print(f"  Model: {model}")
            model_mat_path = os.path.join(base_dir, model, 'results', f'output_mulExm_{index}.mat')
            if not os.path.exists(model_mat_path):
                print(f"    Missing: {model_mat_path}")
                continue

            try:
                I_F = sio.loadmat(model_mat_path)['sr'].astype(np.float32)
                ...
                fig.add_axes(ax)
                ax.set_axis_off()
                im = plt.imshow(HQNR_map, cmap=cmap, vmin=0.75, vmax=1)

                # fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)

                save_name = f'{index:02d}_{model}_HQNR.png'
                save_path = os.path.join(output_dir, save_name)
                plt.savefig(save_path, bbox_inches='tight', pad_inches=0)
                plt.close()

            except Exception as e:
                print(f"    Error processing {model}, index {index}: {e}")

    print("\n✅ 所有模型和 index 处理完成。热力图已保存。")